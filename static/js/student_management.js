...
let editModal;
    ...
    document.addEventListener("DOMContentLoaded", function() {
        initializeDashboard();
        updateContextGreeting(localStorage.getItem('adminName'));
    });
    ...
    async function loadData(resetFilter = false) {
        try {
            ...
            masterTransactions = remapBorrowProofRecords(await tRes.json());
            masterCategories = await cRes.json();
            masterApprovalRecords = remapBorrowProofRecords(await approvalRes.json());
    ...
    function executeUnlock(name, photo, schoolId = '', token = '') {
        ...
        if (sidebarAdminRole) sidebarAdminRole.innerText = 'Authenticated Admin';
        updateContextGreeting(name);
    ...
    function remapBorrowProofRecords(rows) {
        return (Array.isArray(rows) ? rows : []).map((row) => {
            const copy = { ...row };
            if (!copy.proof_borrow_photo && copy.request_id) {
                copy.proof_borrow_photo = `/media/book_borrow_transaction_photos/${copy.request_id}.jpg`;
            }
            return copy;
        });
    }

    function updateContextGreeting(adminName) {
        const greetingNode = document.getElementById('contextGreeting');
        const adminNameNode = document.getElementById('contextAdminName');
        if (!greetingNode || !adminNameNode) return;
        const hour = new Date().getHours();
        const salutation = hour < 12 ? 'Good Morning' : hour < 18 ? 'Good Afternoon' : 'Good Evening';
        adminNameNode.innerText = adminName || localStorage.getItem('adminName') || 'Administrator';
        greetingNode.innerHTML = `${salutation}, <span id="contextAdminName" class="greeting-admin-name">${adminNameNode.innerText}</span>`;
    }

    function showFutureCapability() {
        alert('Future capability under development.');
    }

    function prepareMobileCaptureSession() {
        const requestId = document.getElementById('borrowRequestId')?.value || `REQ-${Date.now().toString(36).toUpperCase()}`;
        const capturePath = `/media/book_borrow_transaction_photos/${requestId}.jpg`;
        const payload = { request_id: requestId, proof_borrow_photo: capturePath, mobile_scanner_ready: true, issued_at: new Date().toISOString() };
        localStorage.setItem('mobileCaptureSession', JSON.stringify(payload));
        const proofInput = document.getElementById('proofBorrowPhoto');
        if (proofInput) proofInput.value = capturePath;
        alert('Mobile camera session prepared. Open this admin session on phone to capture proof.');
    }
    ...
    function openBorrowForm(bookNo) {
        ...
        const proofInput = document.getElementById('proofBorrowPhoto');
        if (proofInput) {
            const req = document.getElementById('borrowRequestId').value;
            proofInput.value = transaction.proof_borrow_photo || `/media/book_borrow_transaction_photos/${req}.jpg`;
        }
    ...
    async function submitBorrowForm() {
        ...
        const proof_borrow_photo = document.getElementById('proofBorrowPhoto')?.value || '';
        const mobile_scanner_ready = Boolean(localStorage.getItem('mobileCaptureSession'));
        if (!return_due_date) return alert('Please set return date.');
        if (!validateBorrowReturnDateSelection()) return;
        try {
            const res = await apiFetch('/api/process_transaction', { method: 'POST', body: JSON.stringify({ book_no: b_no, action: 'borrow', return_due_date, approved_by, request_id, proof_borrow_photo, mobile_scanner_ready }) });
            const data = await res.json();
...
window.showFutureCapability = showFutureCapability;
window.prepareMobileCaptureSession = prepareMobileCaptureSession;
